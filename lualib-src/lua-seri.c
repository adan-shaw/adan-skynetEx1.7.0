...
static const void *rb_read(struct read_block *rb, int sz)
{
	if(rb->len < sz)
	{
		return NULL;
...
static int wb_table_metapairs(lua_State * L, struct write_block *wb, int index, int depth)
{
...
static int wb_table(lua_State * L, struct write_block *wb, int index, int depth)
{
