...
static int get_encodefield (const struct sproto_arg *args)
{
	...
	if (args->index > 0)
	{
		...
		if (args->tagname != self->array_tag)
		{
			...
			{
				return luaL_error (L, ".*%s(%d) should be a table or an userdata with metamethods (Is a %s)", args->tagname, args->index, lua_typename (L, lua_type (L, -1)));
			}
			else
			{
				lua_pushnil (L);
	...
	case SPROTO_TINTEGER:
		{
			...
			else
			{
				...
				if (!isnum)
				{
					return luaL_error (L, ".%s[%d] is not an integer (Is a %s)", args->tagname, args->index, lua_typename (L, lua_type (L, -1)));
				}
			}
			lua_pop (L, 1);
	...
	case SPROTO_TBOOLEAN:
		{
			...
			if (!isbool)
			{
				return luaL_error (L, ".%s[%d] is not a boolean (Is a %s)", args->tagname, args->index, lua_typename (L, lua_type (L, -1)));
			}
			*(int *) args->value = v;
			lua_pop (L, 1);
			return 4;
	...
	case SPROTO_TSTRING:
		{
			...
			if (!isstring)
			{
				return luaL_error (L, ".%s[%d] is not a string (Is a %s)", args->tagname, args->index, lua_typename (L, type));
			}
			if (sz > args->length)
