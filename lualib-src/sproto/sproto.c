...
static uint8_t *encode_array_object (sproto_callback cb, struct sproto_arg *args, uint8_t * buffer, int size, int *noarray)
{
	int sz;
	*noarray = 0;
	args->index = 1;
...
static int decode_empty_array (sproto_callback cb, struct sproto_arg *args)
{
...
static inline void write_ff (const uint8_t * src, const uint8_t * src_end, uint8_t * des, int n)
{
	des[0] = 0xff;
	des[1] = n - 1;
	if (src + n * 8 <= src_end)
