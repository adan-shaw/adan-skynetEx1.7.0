...
static void write_string(struct bson *b, lua_State * L, const char *key, size_t sz)
{
	bson_reserve(b, sz + 1);
...
static inline void append_key(struct bson *bs, lua_State * L, int type, const char *key, size_t sz)
{
	write_byte(bs, type);
