...
static struct channel *channel_release (struct channel *c)
{
	...
	SPIN_UNLOCK (c) SPIN_DESTROY (c) free (c);
	return NULL;
}
...
static struct command *channel_read (struct channel *c, double timeout)
{
	...
	SPIN_LOCK (c) if (c->head == NULL)
	{
		SPIN_UNLOCK (c) int ti = (int) (timeout * 100000);
		usleep (ti);
		return NULL;
	}
	ret = c->head;
