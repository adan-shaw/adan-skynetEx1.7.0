...
struct skynet_context *skynet_context_new (const char *name, const char *param)
{
	...
	CHECKCALLING_BEGIN (ctx) int r = skynet_module_instance_init (mod, inst, ctx, param);
	CHECKCALLING_END (ctx) if (r == 0)
	{
...
static void delete_context (struct skynet_context *ctx)
{
	...
	CHECKCALLING_DESTROY (ctx) skynet_free (ctx);
	context_dec ();
}
...
{
	assert (ctx->init);
	CHECKCALLING_BEGIN (ctx) pthread_setspecific (G_NODE.handle_key, (void *) (uintptr_t) (ctx->handle));
